...
class TwitterOAuth extends Config {
	 ...
	 */
	public function oauth( string $path, array $parameters = array() ): array {
		$response = array();
		$this->resetLastResponse();
		$this->response->setApiPath( $path );
	 ...
	 * @return array|object
	 */
	public function oauth2( string $path, array $parameters = array() ) {
		$method = 'POST';
		$this->resetLastResponse();
		$this->response->setApiPath( $path );
		$url           = sprintf( '%s/%s', self::API_HOST, $path );
	...
	private function request(
		...
		if (
			in_array( $method, array( 'GET', 'PUT', 'DELETE' ) ) &&
			! empty( $postfields )
		) {
